...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (currentSum: number, q: Question): number => currentSum + q.points,
        0,
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (totalSum: number, q: Question): number =>
            totalSum + (q.published ? q.points : 0),
        0,
    );
}
...
export function toCSV(questions: Question[]): string {
    ...
    const lines = questions.map((q: Question): string => {
        return [q.id, q.name, q.options.length, q.points, q.published]
            .map((value: string | number | boolean): string => {
                return String(value);
            })
            .join(",");
    });
...
export function duplicateQuestionInArray(
    ...
    questions.forEach((q: Question): void => {
        output.push(q);
        if (q.id === targetId) {
            output.push(duplicateQuestion(newId, q));
        }
    });
